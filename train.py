...
from Semi_supervise.model import UGAPNet

from Semi_supervise.util import dataset
from Semi_supervise.util import transform, transform_tri, config
from Semi_supervise.util.util import AverageMeter, poly_learning_rate, intersectionAndUnionGPU, get_model_para_number, setup_seed, get_logger, get_save_path, \
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='UGAPNet') #
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='config/pascal/pascal.yaml',
...
# ----------------------  DATASET  ----------------------
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':
        train_data = dataset.SemData(split=args.split, shot=args.shot + args.un_shot, data_root=args.data_root, base_data_root=args.base_data_root, data_list=args.train_list, \
    ...
    if args.evaluate:
        if args.resized_val:
            ...
            val_transform_tri = transform_tri.Compose([
                ...
                transform_tri.Normalize(mean=mean, std=std)])
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset.SemData(split=args.split, shot=args.shot + args.un_shot, data_root=args.data_root, base_data_root=args.base_data_root, data_list=args.val_list, \
...
# ----------------------  TRAINVAL  ----------------------
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and epoch%1==0:
            loss_val, FBIoU, mIoU, mIoU_b, pIoU = validate(val_loader, model)
            val_num += 1
            if main_process() and args.viz:
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        s_input_s = s_input[:, 0:args.shot, :, :, :]
        s_mask_s = s_mask[:, 0:args.shot, :, :]
        s_input_u = s_input[:, args.shot:, :, :, :]

        output, main_loss, aux_loss1, aux_loss2 = model(s_x=s_input_s, s_y=s_mask_s, s_u=s_input_u, x=input, y_m=target, y_b=target_b, cat_idx=subcls)

        loss = main_loss + 0.01*aux_loss1
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b, class_chosen) in enumerate(val_loader):
            ...
            output, base_out,_ = model(s_x=s_input_s, s_y=s_mask_s, s_u=s_input_u, x=input,
    ...
    return loss_meter.avg, mIoU, class_miou, class_miou_b, iou_class[1]

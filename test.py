...
def main():
    ...
    if args.evaluate:
        ...
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset.SemData(split=args.split, shot=args.shot + args.un_shot, data_root=args.data_root,
    ...
    for val_id in range(val_num):
        ...
        fb_iou, miou,  miou_b, piou = validate(val_loader, model, val_seed, visual=viz)
...
def validate(val_loader, model, val_seed, visual):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b, class_id, image_path, label_path, support_image_path_list, support_label_path_list) in enumerate(
                val_loader):
